...
if (window.innerWidth >= 1024) {
    function setSlideshowImages(srcs, alts, $wrapper) {
        $wrapper.append('<div class="card-slideshow-wrapper"></div>');
        for (var i = 0; i < srcs.length; i++) {
            $wrapper.find('.card-slideshow-wrapper').append(`<img class="img-responsive card-slideshow-img imgL${srcs.length}" src="${srcs[i]}" alt="${alts[i]}"/>`);
        }
    }
    ...
    $('.woody-component-focus .card.has-slideshow').each(function () {
        ...
        var final_srcs = [];
        var alts = JSON.parse(atob($this.find('.slideshow-srcs').data('alts'), true));
        ...
        if (cardSlideshowActive == false) {
            ...
            setSlideshowImages(final_srcs, alts, $imgWrapper);
        }
